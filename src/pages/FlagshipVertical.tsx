...
function VerticalPage({ slug }: { slug: VerticalSlug }) {
  ...
  return <div className={`stm-studio flagship-vertical vertical-${slug}`}>
    <StudioHeader />
    <main>
      <section className="vertical-hero"><div className="studio-container vertical-hero-grid"><div className="vertical-hero-copy"><p className="studio-eyebrow">{config.eyebrow}</p>{biz && <p className="vertical-biz-note">A growth plan for {biz}</p>}<h1 className="studio-display">{config.title}</h1><p className="vertical-hero-lede">{config.description}</p><div className="vertical-hero-actions"><a className="studio-btn studio-btn-primary" href="#diagnostic">{config.primaryLabel} <ArrowDown aria-hidden size={16}/></a><Link className="studio-btn studio-btn-outline" to={config.heroProofPath ?? freeAudit}>{config.heroProofLabel ?? config.secondaryLabel}</Link>{config.tertiaryLabel && <Link className="vertical-text-link" to={freeAudit}>{config.tertiaryLabel} <ArrowRight aria-hidden size={15}/></Link>}</div>{segmentSelector}</div><VerticalHeroVisual slug={slug}/></div></section>
      ...
      <section className="vertical-growth studio-section"><div className="studio-container"><p className="studio-eyebrow">The growth system</p><div className="vertical-section-heading"><h2 className="studio-display">Every handoff should strengthen the next one.</h2><p>Strategy, creative, marketing, conversion, and operations work better when they share one commercial goal.</p></div><div className="vertical-growth-grid">{stages.map((stage, index) => <article key={stage.title} className={requestedFocus && config.needs[index]?.id === requestedFocus ? "is-emphasized" : ""}><span>0{index + 1}</span><h3>{stage.title}</h3><p>{stage.summary}</p><ul>{stage.items.map((item) => <li key={item}>{item}</li>)}</ul></article>)}</div></div></section>
      ...
      <VerticalProof config={config} biz={biz}/>

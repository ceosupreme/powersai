      ...
      <main className="min-h-screen bg-[hsl(var(--bone))] font-body text-[hsl(var(--ink))]">
        <div className="mx-auto max-w-4xl px-6 py-16 md:py-24">
          <header className="mb-10">
            ...
            <p className="mt-6 max-w-2xl text-lg text-[hsl(var(--ink))]/70">
          ...
          {!running && !audit.redacted && (
            <form onSubmit={onSubmit} className="rounded-2xl border border-[hsl(var(--ink))]/10 bg-white p-6 md:p-8 shadow-sm">
              <div className="grid gap-5 md:grid-cols-2">
                <label className="block">
                  <span className="mb-1.5 block font-mono text-xs uppercase tracking-wider text-[hsl(var(--ink))]/60">Business name *</span>
                  <input
                    required
                    value={businessName}
                ...
                <label className="block">
                  <span className="mb-1.5 block font-mono text-xs uppercase tracking-wider text-[hsl(var(--ink))]/60">City *</span>
                  <input
                    required
                    value={city}
                    ...
                    {FOOTPRINTS.map((f) => (
                      <button
                        ...
                        className={`rounded-full px-4 py-2 text-sm transition ${
                          footprint === f.value
                            ? 'bg-[hsl(var(--green))] text-[hsl(var(--bone))]'
                            : 'border border-[hsl(var(--ink))]/15 bg-white text-[hsl(var(--ink))] hover:border-[hsl(var(--green))]'
                        }`}
                      >
                        {f.label}
                      </button>
              ...
              <div className="mt-8 flex items-center justify-between">
                <button
                  type="submit"
                  disabled={!canSubmit}
                  className="inline-flex items-center gap-2 rounded-full bg-[hsl(var(--green))] px-7 py-3.5 text-sm font-medium text-[hsl(var(--bone))] transition-all hover:-translate-y-0.5 hover:shadow-xl disabled:opacity-60"
                >
                  {audit.submitting ? 'Starting…' : 'Run my free audit'}
                </button>
                <span className="hidden font-mono text-xs text-[hsl(var(--ink))]/50 md:inline">~30 seconds</span>
              </div>
              {audit.error && <p className="mt-4 text-sm text-[hsl(var(--rust))]">{audit.error}</p>}
            </form>
          )}

          {/* Progress theater */}
          {(running || audit.status === 'failed') && (
            <div className="mt-2 rounded-2xl bg-[hsl(var(--green))] p-6 font-mono text-sm text-[hsl(var(--bone))] shadow-lg md:p-8">
              <p className="mb-4 text-xs uppercase tracking-[0.2em] text-[hsl(var(--bone))]/70">Live read · {businessName || 'your business'}</p>
          ...
          {audit.redacted && !audit.full && (
            <div className="mt-2 space-y-8">
              <div className="rounded-2xl border border-[hsl(var(--ink))]/10 bg-white p-8 md:p-10 shadow-sm">
                <p className="mb-3 font-mono text-xs uppercase tracking-[0.2em] text-[hsl(var(--green))]">Estimated revenue leak</p>
                <p className="font-display text-6xl md:text-8xl leading-[0.95] text-[hsl(var(--rust))]">
                  {fmtMoney(audit.redacted.total_monthly_dollars)}
                  <span className="ml-2 text-2xl md:text-3xl text-[hsl(var(--ink))]/50">/mo</span>
                </p>
                <p className="mt-4 text-lg text-[hsl(var(--ink))]/70">
                ...
                {audit.redacted.top_leaks.length > 0 && (
                  <div className="mt-6 flex flex-wrap gap-2">
                    {audit.redacted.top_leaks.map((n) => (
                      <span key={n} className="rounded-full border border-[hsl(var(--gold))] bg-[hsl(var(--gold))]/10 px-3 py-1 text-sm">{n}</span>
                    ))}
                  </div>
                )}
                {audit.redacted.project_type_resolution?.path === 'default' && (
                  <p className="mt-6 rounded-lg border border-[hsl(var(--gold))]/40 bg-[hsl(var(--gold))]/10 p-3 text-xs">
              ...
              {/* Locked rows */}
              <div className="rounded-2xl border border-dashed border-[hsl(var(--ink))]/15 bg-white/50 p-6">
                <div className="space-y-2 blur-sm select-none pointer-events-none">
                  {Array.from({ length: Math.max(2, audit.redacted.leak_count - 3) }).map((_, i) => (
                    <div key={i} className="flex justify-between border-b border-[hsl(var(--ink))]/10 pb-2">
              ...
              {/* Unlock */}
              <form onSubmit={onUnlock} className="rounded-2xl bg-[hsl(var(--ink))] p-8 text-[hsl(var(--bone))] shadow-lg md:p-10">
                <h3 className="font-display text-3xl md:text-4xl">Unlock your full leak ledger</h3>
                <p className="mt-2 text-[hsl(var(--bone))]/70">Every leak, every dollar, every input source flag. Emailed to you and shown right here.</p>
                <div className="mt-6 grid gap-3 md:grid-cols-[1fr_1fr_auto]">
                  <input
                    ...
                    className="rounded-lg border border-[hsl(var(--bone))]/20 bg-transparent px-3 py-2.5 text-[hsl(var(--bone))] placeholder-[hsl(var(--bone))]/40 focus:border-[hsl(var(--gold))] focus:outline-none"
                    maxLength={200}
                  />
                  <input
                    type="email"
                  ...
                  <button
                    ...
                    className="rounded-full bg-[hsl(var(--gold))] px-6 py-2.5 text-sm font-medium text-[hsl(var(--ink))] disabled:opacity-60"
                  >
          ...
          {audit.full && (
            <div className="mt-2 space-y-8">
              <div className="rounded-2xl border border-[hsl(var(--ink))]/10 bg-white p-8 md:p-10 shadow-sm">
                <p className="mb-3 font-mono text-xs uppercase tracking-[0.2em] text-[hsl(var(--green))]">Full ledger</p>
                <p className="font-display text-5xl md:text-7xl leading-[0.95] text-[hsl(var(--rust))]">
                  {fmtMoney(audit.full.total_monthly_dollars)}
                  <span className="ml-2 text-2xl md:text-3xl text-[hsl(var(--ink))]/50">/mo captured revenue</span>
                </p>
                {audit.full.total_risk_exposure_dollars > 0 && (
                  <p className="mt-2 text-lg text-[hsl(var(--ink))]/60">
                ...
                {audit.full.project_type_resolution?.path === 'default' && (
                  <p className="mt-4 rounded-lg border border-[hsl(var(--gold))]/40 bg-[hsl(var(--gold))]/10 p-3 text-xs">
              ...
              <ul className="space-y-3">
                {audit.full.results.map((leak, i) => (
                  <li key={leak.name + i} className="rounded-2xl border border-[hsl(var(--ink))]/10 bg-white p-5 md:p-6">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className={`rounded-full px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider ${leak.severity === 'headline' ? 'bg-[hsl(var(--rust))] text-[hsl(var(--bone))]' : 'bg-[hsl(var(--ink))]/10 text-[hsl(var(--ink))]/70'}`}>
                            {leak.severity}
                          </span>
                          <span className={`rounded-full px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider ${leak.risk_type === 'avoided_loss' ? 'bg-[hsl(var(--gold))]/20 text-[hsl(var(--ink))]/70' : 'bg-[hsl(var(--green))]/10 text-[hsl(var(--green))]'}`}>
                            {leak.risk_type === 'avoided_loss' ? 'avoided loss' : 'captured revenue'}
                          </span>
                        </div>
                        <h4 className="mt-2 font-display text-xl">{leak.name}</h4>
                        {leak.benchmark && (
                          <p className="mt-1 text-sm text-[hsl(var(--ink))]/60">{leak.benchmark}</p>
                        )}
                      </div>
                      <p className="font-display text-2xl text-[hsl(var(--rust))]">
                        {leak.monthly_dollars == null ? '—' : `${fmtMoney(leak.monthly_dollars)}/mo`}
                      </p>
                    </div>
                    {leak.reason && <p className="mt-3 text-sm text-[hsl(var(--ink))]/70">{leak.reason}</p>}
                    {leak.inputs?.length > 0 && (
                      <details className="mt-3">
                        <summary className="cursor-pointer font-mono text-xs uppercase tracking-wider text-[hsl(var(--ink))]/50">Inputs & sources</summary>
                        <ul className="mt-2 space-y-1 text-xs">
                          {leak.inputs.map((inp, j) => (
                            <li key={j} className="flex flex-wrap gap-x-3 text-[hsl(var(--ink))]/70">
                              <span className="font-mono">{inp.name}</span>
                              <span>= {inp.value ?? '—'}</span>
                              <span className="font-mono text-[10px] uppercase text-[hsl(var(--ink))]/40">{inp.source ?? 'unknown'}</span>
                              {inp.caveat && <span className="w-full text-[hsl(var(--ink))]/50">↳ {inp.caveat}</span>}
                            </li>
                          ))}
                        </ul>
                      </details>
              ...
              {audit.full.competitor_block && (
                <div className="rounded-2xl border border-[hsl(var(--ink))]/10 bg-white p-6 md:p-8">
                  <p className="mb-3 font-mono text-xs uppercase tracking-[0.2em] text-[hsl(var(--green))]">You vs the top 3 near you</p>
                  <p className="mb-4 text-sm text-[hsl(var(--ink))]/60">Search: <span className="font-mono">{audit.full.competitor_block.keyword}</span></p>
                  <ul className="space-y-2 text-sm">
                    <li className="flex justify-between border-b border-[hsl(var(--ink))]/10 pb-2">
                    ...
                    {(audit.full.competitor_block.top_competitors || []).slice(0, 3).map((c: any, i) => (
                      <li key={i} className="flex justify-between border-b border-[hsl(var(--ink))]/10 pb-2 text-[hsl(var(--ink))]/70">
              ...
              <a
                ...
                className="mt-4 block rounded-2xl bg-[hsl(var(--green))] p-8 text-center text-[hsl(var(--bone))] shadow-lg md:p-12"
              >
                <p className="font-display text-2xl md:text-3xl leading-snug">{closingCta}</p>
                <span className="mt-4 inline-block rounded-full bg-[hsl(var(--bone))] px-6 py-2 text-sm font-medium text-[hsl(var(--green))]">Book 15 minutes →</span>
              </a>
            </div>
          )}

          <p className="mt-16 border-t border-[hsl(var(--ink))]/10 pt-6 font-mono text-xs text-[hsl(var(--ink))]/50">

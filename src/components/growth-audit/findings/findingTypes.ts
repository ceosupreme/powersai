...
export const FINDING_TYPE_TEMPLATES: Record<FindingType, FindingTypeTemplate> = {
  ...
  event_underperformance: {
    ...
    category: 'events',
    defaultTrafficDriving: false,
    diagnosisPattern: '[Event] occurs [frequency] but revenue is similar to non-event nights',
    recommendedActionPattern: 'Change [theme/offer/timing] or discontinue [event]',
    evidenceHints: ['Toast — Daily metrics', 'Event calendar'],
  ...
  reputation_risk: {
    ...
    category: 'reputation',
    defaultTrafficDriving: false,
    diagnosisPattern: 'Reviews mention [issue] in [shift/area] — operational concern',
    recommendedActionPattern: 'Flag [shift/area] for ops fix; do not push traffic until resolved',
    evidenceHints: ['Review sentiment classifier', 'Toast KDS ticket times', '7shifts schedule'],
  ...
  operational_readiness_blocker: {
    ...
    defaultTrafficDriving: false,
    diagnosisPattern: '[Shift] shows demand but labor/service indicators suggest capacity strain',
    recommendedActionPattern: 'Fix [staffing/workflow] before marketing; never auto-push traffic',
    evidenceHints: ['7shifts schedule', 'Toast KDS ticket times', 'Review sentiment classifier'],
